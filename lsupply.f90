    ...
    do ium=1,nw
        do iuf=1,nw
            ...
            if(dum6>10d0*(ERREL**2)) then
                ...
                XGUESS2(2)=0.01d0
                ITMAX=1000000
                call getHybrSoln(XGUESS2, 2, labor2_hybrd, sol_loc, fnorm)
                if(FNORM<dum6) then
                    dum4=sol_loc(1)
                    dum5=sol_loc(2)
                    dum6=FNORM
            ...
            elseif((dum4>1d0).AND.(dum5<1d0)) then
                ...
                if(dum6>1d-4) then
                    ...
                    if (verbose) then
                        if (dum6>1d-4) then
                            !print *, 'reoptimisation failed (married corner)'
                        else
                            !print '(a, i4, a, ES9.2)', '(married corner) successfully reoptimized in ', i_reopt-1, ' tries, fnorm = ', dum6
                        end if 
                    end if
                end if                
            elseif((dum4<1d0).AND.(dum5>1d0)) then
