...
contains
    ...
    function fun_opt_mar(cons, data_aux) result(val)
        use Utilities
        use PolicyFunctions
        use ValueFunctions
        use GoldenSearch_mod
        ...
        if (kp < 0.0001d0) then
            ...
            !vprime = val_mar_T(kp, expm_prime_mod, expf_prime_mod, val_fn_ptr)
            vprime = val_mar_T(kp, expf_prime_loc, expm_prime_loc, data_aux%val_fn_ptr)
            Ucur = Uc(cons) + Ul(hm, hf)
            val = Ucur - fc_loc + beta*OmegaActive(T)*vprime
        end if
    ...
    end function fun_opt_mar    
    
    
end module SolveActive
